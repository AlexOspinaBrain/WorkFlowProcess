...
class DefaultController extends Controller
{
	...
	public function actionConsultaTramitesExcel(){
		...
		$model->unsetAttributes();  
			...
			//var_dump($_GET['Radica']);
			//Yii::app()->end();
		
		if(isset($_GET['Radica'])){

			$model->attributes=$_GET['Radica'];
			//if ($_GET['estadoq']!=null)
				//$model->status=$_GET['estadoq'];
	...
	public function actionFormEstudio($id_radica)
   	{
		...
		$usuyaco=0;
   		...
   		if ($modelRadica->status!='swRadica/en_estudio' && $modelRadica->status!='swRadica/en_devolucion' && $modelRadica->status!='swRadica/compromiso'){
   			Yii::app()->user->setFlash('nootusu','No se completó el proceso para el caso '.$id_radica.', ya el caso tenia otro estado.');
   			$this->redirect(array('consultaTramites'));
   		}

   		//marca para que no se seleccione dos veces
   		if ($modelRadica->en_trabajo==null){
   			$modelRadica->en_trabajo=new CDbExpression('NOW()');
   			$modelRadica->usuario_cod_trb = Yii::app()->user->id;
   			$modelRadica->save();
   		}else{
   			//para bloqueo
   			//$usuyaco=1;
		...
		$modelRadica->scenario='estudio';
   		...
   		if($usuyaco==1 && $modelRadica->usuario_cod_trb == Yii::app()->user->id)
   			$usuyaco=0;
        ...
        if ($archval==0 && $usuyaco==0){
         if(isset($_POST['Radica'])){        	
			...
			$modelRadica->attributes=$_POST['Radica'];
			if ($_POST['Radica']['fecha_formulario']=='')
				$modelRadica->fecha_formulario = null;			
			$modelRadica->en_trabajo=null;
			//$modelRadica->usuario_cod_trb=null;
			...
			Yii::app()->user->setFlash('caok','Caso '.$id_radica.' procesado.');
			$this->redirect(array('consultaTramites'));
		 }
		}else{
		    	$modelRadica->validate();
