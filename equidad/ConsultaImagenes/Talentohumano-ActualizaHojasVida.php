	...
	<script type="text/javascript">
		...
		function tables(){
			$(".flexme3").flexigrid({
				...
				dataType : 'xml',
				params:		[ { name: 'consulta', value: 'ActualizaHojasVida' }, { name: 'Usuario', value: <?=$_SESSION['uscod']?> }							  
							],
		...
		function CambiaFiltros(){
			$(".flexme3").flexOptions(
				{params:[{ name: 'consulta', value: 'ActualizaHojasVida' }, 
		...
		function ActualizaHV(numid){			
			$.ajax({
					type: "POST",
					url: "ConsultaImagenes/ajax_querys.php",
					data: { op: "ActualizaHV", term: $(numid).attr("id"), Estado: $(numid).val()}
				}).done(function( data ) {	
					var obj =$.parseJSON(data);
					if(obj[0].value > 0)
						alert("Actualizado correctamente");
					else{
						alert("No se ha podido actualziar");
						alert(data);
					}
				});
		}
