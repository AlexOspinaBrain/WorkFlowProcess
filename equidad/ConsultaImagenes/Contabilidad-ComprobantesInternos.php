	...
	<script type="text/javascript">
        $(document).ready(function() {	
			...
			$('#Borrar').click(function(){
				$('#Desde, #Hasta, #Compania, #Comprobante').val('');
				CambiaFiltros();
			});
			$('#Desde, #Hasta, #Compania, #Comprobante').keypress(function(e) {
				if(e.which == 13) 
					CambiaFiltros();				
			});
		...
		function tables(){
			$(".flexme3").flexigrid({
				...
				dataType : 'xml',
				params:		[ { name: 'consulta', value: 'ComprobantesInternos' }							  
							],
				colModel : [ { display : 'Fecha',	name : 'fecha', width : 80, sortable : true, align : 'center'}, 
							 ...
							 { display : 'Comprobante', name : 'agencia', width : 250, sortable : true,	align : 'left'}, 
		...
		function ExportExcel(){
			parametros="consulta="+"ComprobantesInternos";
...
<table align='center'><tr><td>
	...
	<fieldset style="width:170px; height:50px;" class='filtros'>
		<legend><b>Comprobante: </b></legend>			
		<input type="text" id="Comprobante"/><br>
	</fieldset>
...
<?=OpcionesSelect('Compania', 'planillascontabilidad pla', 'distinct(compañia)', 'pla.compañia', 'where pla.cab=50')?>
