...
function MuestraPermisos($idUsu){	
	...
	$conect=new conexion();
	$conect2=new conexion();
	$consulta=$conect->queryequi("select * from adm_menu where jerarquia_opcion like '4.1%'");
	while ($row = pg_fetch_array($consulta)){
		...
		$consulta2=$conect2->queryequi("select count(*) from adm_usumenu um where um.jerarquia_opcion='".$row['jerarquia_opcion']."' and um.usuario_cod='$idUsu'");
		$row2 = pg_fetch_array($consulta2);
		//$opciones[$salida]=array('jerarquia_opcion' => str_replace("4.", "",$row['jerarquia_opcion']), 'opcion' => $row['opcion'], 'selec' => $row2[0]);	
		$opciones[$salida]=array('jerarquia_opcion' => substr($row['jerarquia_opcion'],2), 
			'opcion' => $row['opcion'], 'selec' => $row2[0]);	
	}
...
function GuardaPermisos($guardar){
	...
	$conect=new conexion();
	$consulta=$conect->queryequi("select * from adm_menu where jerarquia_opcion like '4.%'");
	$permisos="";
	while ($row = pg_fetch_array($consulta)){
		$jerarquia=str_replace(".", "-", $row['jerarquia_opcion']);
		if($_REQUEST[str_replace("4-", "", $jerarquia)]=='on')
			$permisos.="('".$_REQUEST["usuario_cod"]."', '".$row['jerarquia_opcion']."'), ";
	}
	
	$permisos=substr($permisos, 0, strlen($permisos)-2); 
	$conect->queryequi("delete from adm_usumenu where usuario_cod='".$_REQUEST["usuario_cod"]."' and jerarquia_opcion like '4.%'");

	if(strlen($permisos)>0)
		$conect->queryequi("INSERT INTO adm_usumenu(usuario_cod, jerarquia_opcion) VALUES ".$permisos);
