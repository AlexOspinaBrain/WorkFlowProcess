	...
	<script type="text/javascript">
		$(document).ready(function() {	
			$("#ListRadica").jqGrid({
				...
				postData: {
				 	query: 'ConsultaCP',
				    estado: ['Recibir Auditoria', 'Recibir corrección Comprobante de pago', 'Recibir para cierre'],
				    usuarioPend: 'login',
				...
				colModel:[
					...
					{name:'num_comprobante',index:'num_comprobante', width:7, align:"center", formatter:'showlink', formatoptions:{baseLinkUrl:'#'}},
					...
					{name:'cant_ord',index:'cant_ord', width:3, search:false}
				],
				scroll:1,
				rowNum:50,
				...
				viewrecords: true,
				sortorder: "desc",
				caption:"Comprobantes de pago pendientes por recibir",
				altRows:true,
				altclass:'altClassrow',
				gridComplete: function (){
					$("#ListRadica td[aria-describedby='ListRadica_num_comprobante'] a").click(function (){DetallesCP($(this).text());})
					$("#ListRadica td[aria-describedby='ListRadica_LinkRecibe']").click(function (){RecibeCP($(this).parent().attr("id"));})
		...
		function RecibeCP(tramite){
			$.ajax({
				type: "POST",
				url: "Facturacion/views/escritorio/recibeCP.php",
				data: { tramite: tramite},
				success	:function (data){
					$( "body" ).append(data);	
				}
