...
if($_POST['consulta'] == 'ActualRadica'){	
        ...
        $where = " from trasacorrespondencia tra, radcorrespondencia cor, tblradofi ofi, tblareascorrespondencia are, tbltiposdoccorresp doc where cor.fecins >= (now() + '-30 day') and cor.radicado='".$_REQUEST['codusu']."' and tra.estado='RADICADO' and not exists(select * from trasacorrespondencia where numtramite=cor.numtramite and estado!='RADICADO' and fechahora is not null  ) and tra.numtramite=cor.numtramite and cor.tipodoc=doc.tipodocid and cor.area=are.areasid and are.agencia=ofi.codigo";
...
if($_POST['consulta'] == 'LotesPlanillas'){
        ...
        $where = " from radcorrespondencia cor, trasacorrespondencia tra, tblareascorrespondencia are, tblradofi ofi, tbltiposdoccorresp doc where tra.estado!='CERRADO' and tra.area='".$_REQUEST['areausu']."' and fechahora is not null and not exists (select * from trasacorrespondencia where numtramite=tra.numtramite and fechahora>tra.fechahora) and cor.numtramite=tra.numtramite and are.areasid=cor.area and cor.tipodoc=doc.tipodocid and cor.tipodoc=doc.tipodocid and are.agencia=ofi.codigo  group by tra.numtramite, cor.fecins, doc.tipo, cor.numfolios, cor.destinatario, cor.numtramite, cor.remitente, are.area, ofi.descrip";
...
if($_POST['consulta'] == 'Escritorio'){	
	...
	$consulta="select $OpcionEscritorio $Detalles to_char(cor.fecins,'yyyy-MM-dd HH:MI:SS AM') as fecha, ((select (case when usuario_cod=cor.remitente then nombres else cor.remitente end) as remitente from usuarios_tmp order by usuario_cod=cor.remitente desc limit 1)), ofi.descrip, are.area, (case when cor.destinatario='0' then (select destinatario from radcorresext where numtramite=cor.numtramite ) else (select nombres from usuarios_tmp where usuario_cod=cor.destinatario) end), doc.tipo, cor.numfolios, cor.identificacion ";
        ...
        $where = " from radcorrespondencia cor, trasacorrespondencia tra, tblradofi ofi, tblareascorrespondencia are, tbltiposdoccorresp doc where tra.area='".$_REQUEST['areausu']."' $FiltroEstado and cor.numtramite=tra.numtramite and cor.tipodoc=doc.tipodocid and cor.area=are.areasid and are.agencia=ofi.codigo";
        $campoCount = "cor.numtramite";
        ...
        if($_REQUEST['FiltroEstado'] == 5){
                ...
                $where = " from radcorrespondencia cor, trasacorrespondencia tra, tblradofi ofi, tblareascorrespondencia are, tbltiposdoccorresp doc where cor.area='".$_REQUEST['areausu']."' and tra.estado='RADICADO' and not exists(select * from trasacorrespondencia where numtramite=cor.numtramite and fechahora is not null and area='".$_REQUEST['areausu']."') and not exists(select * from trasacorrespondencia where numtramite=cor.numtramite and ((fechahora is not null and area='".$_REQUEST['areausu']."') or estado='ELIMINADO')) and cor.numtramite=tra.numtramite and cor.tipodoc=doc.tipodocid and cor.area=are.areasid and are.agencia=ofi.codigo";
        }
...
if($_POST['consulta'] == 'ConsultaTramite'){
	...
	if($_REQUEST['NumTramite'] != null)
		$NumTramite=" cor.numtramite ='".$_REQUEST['NumTramite']."'";
		
	if($_REQUEST['Asunto'] != null)
		$NumTramite=" cor.asunto ilike '%".$_REQUEST['Asunto']."%'";
		
	if($_REQUEST['Desde'] != null || $_REQUEST['Hasta'] != null){
		if($NumTramite==null)
			$Fecha=" to_char(cor.fecins,'yyyyMMdd')='".$_REQUEST['Desde'].$_REQUEST['Hasta']."'";
		else
	...
	if($_REQUEST['Desde'] != null && $_REQUEST['Hasta'] != null){
		if($NumTramite==null)
			$Fecha=" cor.fecins between '".$_REQUEST['Desde'] ."' and (date '".$_REQUEST['Hasta'] ."'+ interval '1 day')";
		else
			$Fecha="and cor.fecins between '".$_REQUEST['Desde'] ."' and (date '".$_REQUEST['Hasta'] ."'+ interval '1 day')";
	}
		
	if($_REQUEST['Estado'] != null){
		if($NumTramite!=null || $Fecha!=null)
			$Estado="and ";
		else
			$Estado="";

		if($_REQUEST['Estado'] == 1)
			$Estado.=" cor.numtramite in (select numtramite from trasacorrespondencia where (estado='RECIBIDO CORRESPONDENCIA' or estado='RECIBIDO') and fechahora is null)";
		
		if($_REQUEST['Estado'] == 2)
			$Estado.=" cor.numtramite in (select numtramite from trasacorrespondencia where estado='ENVIADO' and fechahora is null)";
			
		if($_REQUEST['Estado'] == 3)
			$Estado.=" cor.numtramite in (select numtramite from trasacorrespondencia where estado='CERRADO' and fechahora is null)";
		
		if($_REQUEST['Estado'] == 4)
			$Estado.=" cor.numtramite in (select numtramite from trasacorrespondencia tras where tras.estado='DEVUELTO' and tras.fechahora=(select max(fechahora) from trasacorrespondencia where numtramite=tras.numtramite))";
			
		if($_REQUEST['Estado'] == 5)
			$Estado.=" cor.numtramite in (select numtramite from trasacorrespondencia tras where tras.estado='REDIRECCIONADO' and tras.fechahora=(select max(fechahora) from trasacorrespondencia where numtramite=tras.numtramite))";
			
		if($_REQUEST['Estado'] == 6)
			$Estado.=" cor.numtramite in (select numtramite from trasacorrespondencia where estado='CERRADO' and fechahora is not null)";
	...
	if($_REQUEST['AreaOrigen'] != null){
		if($NumTramite!=null || $Fecha!=null || $Estado!=null)
			...
			$AreaOrigen=" exists(select numtramite from trasacorrespondencia where estado='RADICADO' and numtramite=cor.numtramite and area='".$_REQUEST['AreaOrigen']."')";
	}

	if($_REQUEST['AreaDestino'] != null){
		if($NumTramite!=null || $Fecha!=null || $Estado!=null || $AreaOrigen!=null)
			$AreaDestino="and cor.area ='".$_REQUEST['AreaDestino']."'";
		else
			$AreaDestino=" cor.area ='".$_REQUEST['AreaDestino']."'";
	}
		
	if($_REQUEST['Siniestro'] != null){
		if($NumTramite!=null || $Fecha!=null || $Estado!=null || $AreaOrigen!=null || $AreaDestino!=null)
			$Siniestro="and cor.siniestro ='".$_REQUEST['Siniestro']."'";
		else
			$Siniestro=" cor.siniestro ='".$_REQUEST['Siniestro']."'";
	}
	...
	$where = " from radcorrespondencia cor, tblradofi ofi, tblareascorrespondencia are, tbltiposdoccorresp doc where $NumTramite $Fecha $Estado $AreaOrigen 
		$AreaDestino $Siniestro and cor.tipodoc=doc.tipodocid and cor.area=are.areasid and are.agencia=ofi.codigo";
	
	$campoCount = "cor.numtramite";
}

if($_POST['consulta'] == 'ConsultaSiniestro'){
	$Siniestro=null;
	$Fecha=null;
	$AreaOrigen=null;
	...
	if($_REQUEST['AreaOrigen'] != null){
		if ($Siniestro!=null || $Fecha!=null)
			$AreaOrigen="and exists(select numtramite from trasacorrespondencia where estado='RADICADO' and numtramite=cor.numtramite and area='".$_REQUEST['AreaOrigen']."')";
		else
			$AreaOrigen=" exists(select numtramite from trasacorrespondencia where estado='RADICADO' and numtramite=cor.numtramite and area='".$_REQUEST['AreaOrigen']."')";
	}
	
	$detsin="'<a href=\"#\" class=\"Detalles\" title=\"Actualiza Siniestro\" onClick=\"CambiaSiniestro('|| cor.numtramite ||')\"><img src=\"images/edit.png\" width=\"15px\" border=\"0\"/></a>', ";

	$consulta="select $detsin $Detalles to_char(cor.fecins,'yyyy-MM-dd HH:MI:SS AM') as fecha, ((select (case when usuario_cod=cor.remitente then nombres else cor.remitente end) as remitente from usuarios_tmp order by usuario_cod=cor.remitente desc limit 1)), ofi.descrip, are.area, (case when cor.destinatario='0' then (select destinatario from radcorresext where numtramite=cor.numtramite ) else (select nombres from usuarios_tmp where usuario_cod=cor.destinatario) end), doc.tipo, cor.numfolios , cor.siniestro ";
	$where = " from radcorrespondencia cor, tblradofi ofi, tblareascorrespondencia are, tbltiposdoccorresp doc 
		where $Siniestro $Fecha $AreaOrigen and LENGTH(cor.siniestro) > 1 and cor.tipodoc=doc.tipodocid and cor.area=are.areasid and are.agencia=ofi.codigo";
	$campoCount = "cor.numtramite";
}
...
if($_POST['consulta'] == 'TramitesArea'){	
	...
	$consulta="select $OpcionEscritorio $Detalles to_char(cor.fecins,'yyyy-MM-dd HH:MI:SS AM') as fecha, ((select (case when usuario_cod=cor.remitente then nombres else cor.remitente end) as remitente from usuarios_tmp order by usuario_cod=cor.remitente desc limit 1)), ofi.descrip, are.area, (case when cor.destinatario='0' then (select destinatario from radcorresext where numtramite=cor.numtramite ) else (select nombres from usuarios_tmp where usuario_cod=cor.destinatario) end), doc.tipo, cor.numfolios, cor.identificacion ";
	$where = " from radcorrespondencia cor, tblradofi ofi, tblareascorrespondencia are, tbltiposdoccorresp doc where  exists(select * from trasacorrespondencia where numtramite=cor.numtramite and fechahora is null and (estado='CERRADO' or estado='RECIBIDO DESTINATARIO') $Area $NumTramite) and  cor.tipodoc=doc.tipodocid and cor.area=are.areasid and are.agencia=ofi.codigo";
	$campoCount = "cor.numtramite";
}
...
if($_POST['consulta'] == 'Informe'){
	...
	if($_REQUEST['AreaDestino'] != null){
		if($NumTramite!=null || $Fecha!=null || $AreaOrigen!=null)
			$AreaDestino="and cor.area ='".$_REQUEST['AreaDestino']."'";
		else
			$AreaDestino=" cor.area ='".$_REQUEST['AreaDestino']."'";
	}
	
	if ($NumTramite==null && $Fecha==null && $AreaOrigen==null && $AreaDestino==null){
	
		$consulta="select $Detalles to_char(cor.fecins,'yyyy-MM-dd HH:MI:SS AM') as fecha, ((select (case when usuario_cod=cor.remitente then nombres else cor.remitente end) as remitente from usuarios_tmp order by usuario_cod=cor.remitente desc limit 1)) as remitente, ofi.descrip, are.area, (case when cor.destinatario='0' then (select destinatario from radcorresext where numtramite=cor.numtramite ) else (select nombres from usuarios_tmp where usuario_cod=cor.destinatario) end), doc.tipo, cor.numfolios, cor.identificacion ";
		$where = " from radcorrespondencia cor, tblradofi ofi, tblareascorrespondencia are, tbltiposdoccorresp doc where cor.numtramite = '1' and cor.tipodoc=doc.tipodocid and cor.area=are.areasid and are.agencia=ofi.codigo ";
		//and LENGTH(cor.numtramite)=15 limit 5
		$campoCount = "cor.numtramite";
	}else{

		$consulta="select $Detalles to_char(cor.fecins,'yyyy-MM-dd HH:MI:SS AM') as fecha, ((select (case when usuario_cod=cor.remitente then nombres else cor.remitente end) as remitente from usuarios_tmp order by usuario_cod=cor.remitente desc limit 1)) as remitente, ofi.descrip, are.area, (case when cor.destinatario='0' then (select destinatario from radcorresext where numtramite=cor.numtramite ) else (select nombres from usuarios_tmp where usuario_cod=cor.destinatario) end), doc.tipo, cor.numfolios, cor.identificacion ";
		$where = " from radcorrespondencia cor, tblradofi ofi, tblareascorrespondencia are, tbltiposdoccorresp doc where $NumTramite $Fecha $AreaOrigen 
			$AreaDestino and cor.tipodoc=doc.tipodocid and cor.area=are.areasid and are.agencia=ofi.codigo ";
		//and LENGTH(cor.numtramite)=15 limit 5
		$campoCount = "cor.numtramite";
...
if($_POST['consulta'] == 'TodosLosPermisos'){
	...
	while ($row = $consulta->FetchRow()){
		$permisos.=", '<input type=\"checkbox\" 
			onClick=\"GuardaPermiso(' || usu.usuario_cod || ', ''".$row['jerarquia_opcion']."'', this)\" ' || 
				(case when exists(select * from adm_usumenu where usuario_cod=usu.usuario_cod and jerarquia_opcion='".$row['jerarquia_opcion']."') then 'checked' else '' end) 
			|| '/>' ";
	}
	...
	$where = " from adm_usuario usu where usuario_bloqueado = false $CodUsuario $Permiso";
...
if($_POST['consulta'] == 'AdmAgencias'){
	...
	$opcion = '<img src="images/edit.png" width="15px" border="0" style="cursor: pointer" 
		onClick="editaAgencia(\'\'\'|| ofi.codigo||\'\'\', \'\'\'|| ofi.descrip||\'\'\')">';
...
if($_POST['consulta'] == 'AdmDocumentos'){
	...
	$where = " from tblareascorrespondencia are, tbltiposdoccorresp doc where doc.area=are.areasid 
		$Doc $Area $Prio";
